/*
  ...
  This example:
    ...
    waits until 180 seconds before the next satellite pass;
    instructs the ARTIC to Transmit One Package And Go Idle;
    keeps checking the MCU status until transmit is complete;
    repeats the message transmit five times, 90 seconds apart;
    repeats for the next satellite pass.

  This example transmits up to 5 packages per pass, 90 seconds apart.
...
const uint32_t PLATFORM_ID = 0x015116BE; // Update this with your Platform ID
...
const uint32_t repetitionPeriod = 90; // The delay in seconds between transmits a.k.a. the repetition period (CLS will have told you what your repetition period is)
...
*/
      ...
      Serial.println(nextSatellitePass - (2 * repetitionPeriod));
      Serial.println();
